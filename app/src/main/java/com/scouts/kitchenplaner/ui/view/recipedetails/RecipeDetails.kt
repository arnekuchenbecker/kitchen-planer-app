...
fun RecipeDetails(recipeID: Long, viewModel: EditRecipeViewModel = hiltViewModel()) {
    ...
    if (recipeInitialized) {
        ...
        Scaffold(topBar = {
            EditableHeader(titleField = { strategy.DisplayHeaderField() }, buttonClick = {
                if (!viewModel.isEditable()) {
                    viewModel.activateEditMode(recipe)
                } else {
                    viewModel.deactivateEditMode()
                }
            }, buttonImage = { strategy.DisplayHeaderImage() })
            ...
            Column(
                ...
                ContentBox(title = "Allergene", modifier = Modifier.padding(10.dp)) {
                    ExpandableCard(expanded = viewModel.expandedFreeOf, onCardArrowClick = {
                        ...
                        DisplayDietarySpecialityList(editable = { viewModel.isEditable() },
