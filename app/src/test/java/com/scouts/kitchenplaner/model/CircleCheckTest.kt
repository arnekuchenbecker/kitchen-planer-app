...
class CircleCheckTest {
    ...
    fun checkForestCreation() {
        val conversions1 = listOf(
            UnitConversion(Regex("Mehl"), "kg", "g", 1000.0),
            UnitConversion(Regex("Mehl"), "g", "Pck", 1 / 1000.0),
            UnitConversion(Regex("Mehl"), "Pck", "kg", 1.0)
        )

        val forest1 = UnitConversionForest(conversions1)

        Assertions.assertEquals(1, forest1.trees.size, "Found an incorrect number of trees")
        Assertions.assertArrayEquals(
            arrayOf(1, 2, 3),
        ...
        val conversions2 = listOf(
            ...
            UnitConversion(Regex("Mehl"), "g", "Pck", 1 / 1000.0),
            UnitConversion(Regex("Mehl"), "Pck", "kg", 1.0),
            UnitConversion(Regex("[a-zA-Z]*"), "kg", "g", 1000.0),
            UnitConversion(Regex("Zucker"), "g", "kg", 1 / 1000.0)
        )

        val forest2 = UnitConversionForest(conversions2)

        Assertions.assertEquals(2, forest2.trees.size, "Found an incorrect number of trees")
        val mehlTree = forest2.trees.find {
            it.conversions.any { conversion -> conversion.pattern == "Mehl" }
        }
        if (mehlTree == null) {
        ...
        val zuckerTree = forest2.trees.find {
            it.conversions.any { conversion -> conversion.pattern == "Zucker" }
        }
        if (zuckerTree == null) {
            Assertions.fail<Unit>("Did not find all trees")
        } else {
    ...
    fun checkEasyUnitConversions() {
        ...
        val conversions2 = listOf(
            UnitConversion(Regex("Mehl"), "kg", "g", 1000.0),
            UnitConversion(Regex("Zucker"), "g", "Pck", 1 / 1000.0),
            UnitConversion(Regex("Mehl"), "Pck", "kg", 1.0)
        )
    ...
    fun checkComplexUnitConversions() {
        val conversions = listOf(
            UnitConversion(Regex("[a-zA-Z]*"), "g", "kg", 1/1000.0),
            UnitConversion(Regex("Milch"), "ml", "l", 1/1000.0),
            UnitConversion(Regex("Mehl"), "kg", "g", 1000.0),
            UnitConversion(Regex("Zucker"), "kg", "Pck", 1.0),
            UnitConversion(Regex("Zucker"), "Pck", "g", 1000.0),
            UnitConversion(Regex("Milch"), "l", "g", 1000.0)
        )
